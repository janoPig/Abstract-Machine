...
namespace
{
	...
	static void AssertRoundtrip(const DslCompiler<typename MachineT::Config>& compiler, const char* dsl)
	{
	...
	TEST_F(FloatVmTest, Compile_EmptySource_AllNops)
	{
		auto c = Compiler();
		VMF::ProgramT prog{};
		CompileOrFail(c, "", (DSLProgramT&)prog);
	...
	protected:
		...
		{
			VMM::ProgramT prog{};
			return Compiler().Compile(dsl, (DSLProgramT&)prog);
		}
	};
	...
	TEST_F(CompoundISetTest, Roundtrip_PrefixedCompound_Stable)
	{
		...
		DslCompilerT c(vm2.GetInstructionSet(), vm2.GetTypeReg());
		AssertRoundtrip<VMM>(c, "Float.Add I[0] I[1]\nInt.Mul I[0] I[1]\n");
	}
	...
	TEST_F(RoundtripPropertyTest, RandomMultiTypePrograms_RoundtripStable)
	{
		...
		for (int it = 0; it < ITER; ++it)
		{
			...
			VMM::ProgramT p1{};
			auto r1 = c.Compile(src, (DSLProgramT&)p1);
			if (!r1) continue;   // invalid programs are expected — skip

			auto d1 = c.Decompile((const DSLProgramT&)p1);
			ASSERT_TRUE(d1.has_value());

			VMM::ProgramT p2{};
			auto r2 = c.Compile(*d1, (DSLProgramT&)p2);
			ASSERT_TRUE(r2) << "Decompiled output failed to recompile:\n" << *d1;

			auto d2 = c.Decompile((const DSLProgramT&)p2);
